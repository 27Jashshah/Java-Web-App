...
@WebServlet("/runDateAfter.html")
public class DateAfterServlet extends HttpServlet
{
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if(!(request.getParameter("dateSearchString").matches("\\d{4}-\\d{2}-\\d{2}"))) {
            ...
            RequestDispatcher dispatch = context.getRequestDispatcher("/dateAfter.jsp");
            dispatch.forward(request, response);
        }
        else {
            List<String> ids = model.dateFilter(request.getParameter("dateSearchString"), "After");
            ...
            ServletContext context = getServletContext();
            RequestDispatcher dispatch = context.getRequestDispatcher("/dateAfterResult.jsp");
            dispatch.forward(request, response);
        }
    }
}