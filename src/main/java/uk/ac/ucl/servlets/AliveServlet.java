...
public class AliveServlet extends HttpServlet
{
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        List<String> ids = model.getAlivePatients();
        request.setAttribute("ids",ids);

        List<String> patientNames = model.getSelectivePatientNames(ids);
        request.setAttribute("patientNames",patientNames);

        request.setAttribute("aliveNumber", Integer.toString(ids.size()));

        ServletContext context = getServletContext();
        RequestDispatcher dispatch = context.getRequestDispatcher("/alive.jsp");
